...
#' @description
#' `timeliner_plot()` takes the aggregated data frame from FDA_Data_for_TimelineR to produce a plotly data visualization of a patient journey
#' @param data is a data frame down stream of FDA_Data_for_TimelineR
#' @return A ggplotly Timeline for a given data set
#' @export

#'
timeliner_plot <- function(.data,
  ...
  start_date <- start_date.a$date - 1000 #StartBuffer
  # Let's define the end of the timeline
  end_date.a <- timeline %>%
    slice_tail()
  # Let's build in a day buffer after the end date
  end_date <- end_date.a$date + 1000 #EndBuffer
